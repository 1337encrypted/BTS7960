...
class BTS7960
{
  ...
  inline BTS7960();
  inline BTS7960(const int8_t& = -1, const int8_t& = -1, const int8_t& = -1, const int8_t& = -1, const uint32_t& = 1000, const uint8_t& = 8, const int8_t& = -1, const int8_t& = -1, const int8_t& = -1, const bool& = false); // L_EN, R_EN, L_PWM, R_PWM, L_IS, R_IS, ID
...
BTS7960::BTS7960(const int8_t& L_EN, const int8_t& R_EN, const int8_t& L_PWM, const int8_t& R_PWM, const uint32_t& freq, const uint8_t& resolution, const int8_t& L_IS, const int8_t& R_IS, const int8_t& ID, const bool& debugStatus) :
L_EN(L_EN),
R_EN(R_EN),
...
void BTS7960::enable()
{
  ...
  Serial.println((String)this->ID+" initilized and enabled");
  return;
}

void BTS7960::disable()
{
  //Setting the BTS7960 enable pins low
  digitalWrite(this->R_EN, LOW);
  digitalWrite(this->L_EN, LOW);

  Serial.println((String)this->ID+" motor driver disabled");
  return;
}
...
void BTS7960::stop()
{
  ...
  Serial.println("Motor "+(String)ID+" STOP: "+this->DEFAULTSPEED);
}
...
void BTS7960::front()
{
  ...
  Serial.println("Motor "+(String)ID+" FRONT: "+this->speed);
}
...
void BTS7960::back()
{
  ...
  Serial.println("Motor "+(String)ID+" BACK: "+this->speed);
}
...
void BTS7960::alarm()
{
  ...
  {
      disable();
      Serial.println("High current alarm");
  }  
}
