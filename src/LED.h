...
class led
{
  ...
  //Function prototypes
  inline led(const int8_t& = -1, const uint32_t& = 1000, const uint8_t& = 8, const String& = "LED") __attribute__((always_inline));
...
led::led(const int8_t& ledPin, const uint32_t& frequency, const uint8_t& resolution, const String& ledId) :
ledPin(ledPin),
ledId(ledId),
...
void led::begin()
{
  ...
  Serial.println(this->ledId+" object initilized");
...
void led::on()
{
  ...
  Serial.println(this->ledId+": ON");
}

void led::off()
{
  ledcWrite(ledPin, DUTYCYCLE0);
  Serial.println(this->ledId+": OFF");
}
...
void led::toggle()
{
  ...
  if(millis() - ledMillis > 700)
  {
    ...
    Serial.println(this->ledId+": Toggle");
  }
}
...
void led:: blinkTwice()
{
  ...
  off();
  vTaskDelay(50 / portTICK_PERIOD_MS);
  Serial.println(this->ledId+": Blink Twice");
}
