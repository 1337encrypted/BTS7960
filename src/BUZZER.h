...
public:
  inline buzzer(const int8_t& = -1, const int8_t& = -1, const uint8_t& = 8, const uint32_t& = 5000) __attribute__((always_inline));
...
buzzer::buzzer(const int8_t& buzzId, const int8_t& buzzPin, const uint8_t& resolution, const uint32_t& frequency) :
buzzId(buzzId),
buzzPin(buzzPin),
...
void buzzer::begin() 
{
  ...
  Serial.println(String(this->buzzId)+" object initilized");
}
...
void buzzer::initBuzzer()
{
  ...
  Serial.println(String(this->buzzId)+": Init Buzzer");
}
...
void buzzer::deinitBuzzer()
{
  ...
  Serial.println(String(this->buzzId)+": deInit Buzzer");
}
...
void buzzer::alarm()
{
  ...
  Serial.println(String(this->buzzId)+": Alarm");
}
